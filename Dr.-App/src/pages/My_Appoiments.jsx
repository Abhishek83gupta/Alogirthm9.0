...
function MyAppointments() {
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen text-white">
      <p className="text-3xl font-bold mb-6 text-center">My Appointments</p>
      {loading ? (
        ...
        <div className="relative border-l border-gray-700">
          {Array.isArray(listdata) && listdata.length > 0 ? (
            listdata.map((doctor, index) => (
              <div
                key={index}
                className="relative flex flex-col sm:flex-row bg-gray-800 shadow-lg rounded-lg p-6 mb-6 ml-6 hover:shadow-xl transition duration-300"
              >
                {/* Timeline Circle */}
                <div className="absolute left-0 top-6 transform -translate-x-1/2">
                  <div className="w-8 h-8 bg-blue-500 rounded-full border-4 border-gray-900 shadow"></div>
                </div>
                {/* Doctor Image */}
                ...
                <div className="sm:w-3/4 sm:ml-6 mt-4 sm:mt-0">
                  ...
                  <div className="mt-3 flex items-center space-x-2">
                    {!doctor.paid && !doctor.cancelled && (
                      <span className="px-3 py-1 text-xs font-medium text-blue-400 bg-gray-700 rounded-full">
                        Upcoming
                      </span>
                    )}
                    {doctor.paid && (
                      <span className="px-3 py-1 text-xs font-medium text-green-400 bg-gray-700 rounded-full">
                        Paid
                      </span>
                    )}
                    {doctor.cancelled && (
                      <span className="px-3 py-1 text-xs font-medium text-red-400 bg-gray-700 rounded-full">
                        Cancelled
                      </span>
                    )}
                  </div>
